...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportActionBar?.title = "Hero Mobile Legends"
    ...
    private fun getListHeroes(): ArrayList<Hero> {
        ...
        val datadamagetype = resources.getStringArray(R.array.data_damagetype)
        val listHero = ArrayList<Hero>()
        for (i in dataName.indices) {
            val hero = Hero(dataName[i], dataDescription[i], dataPhoto.getResourceId(i, -1),dataalias[i],datagender[i],dataspecies[i],datadamagetype[i])
            listHero.add(hero)
        }
        return listHero
    }
